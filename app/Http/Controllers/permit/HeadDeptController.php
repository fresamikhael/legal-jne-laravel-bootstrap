...
class HeadDeptController extends Controller
{
    public function index()
    {
        ...
        foreach ($permit as $key => $value) {
            ...
            $permit[$key]['user_submited'] = User::query()
                ...
                ->firstOrFail()->name;
            if (
                $permit[$key]['history']->to_level == 'HEAD-DEPT' ||
                ($permit[$key]['history']->from_level == 'HEAD-DEPT' &&
        ...
        return view('pages.head-dept.permit.index', [
            'data' => $data,
        ]);
    }
    ...
    public function detail($id)
    {
        ...
        $lastHistory = PermitHistory::where('permit_id', $id)
            ...
            ->limit(1)
            ->first();
        if ($lastHistory->to_level == 'HEAD-DEPT') {
            if ($permit->permit_model == 'permohonan') {
                return view('pages.head-dept.permit.edit_permohonan', [
                    'permit' => $permit,
            ...
            return view('pages.head-dept.permit.edit_perpanjangan', [
                'permit' => $permit,
                'permitHistory' => $permitHistory,
                'permitFile' => $permitFile,
            ]);
        ...
        if ($permit->permit_model == 'permohonan') {
            return view('pages.head-dept.permit.detail_permohonan', [
                'permit' => $permit,
                'permitHistory' => $permitHistory,
                'permitFile' => $permitFile,
            ]);
        ...
        return view('pages.head-dept.permit.detail_perpanjangan', [
            'permit' => $permit,
            'permitHistory' => $permitHistory,
            'permitFile' => $permitFile,
        ]);
    ...
    public function update(Request $request, $id)
    {
        switch ($request->input('action')) {
            case 'return':
                ...
                $dataHistory['from_level'] = 'HEAD-DEPT';
                ...
                return redirect()->route('headdept.permit.index');
                break;
